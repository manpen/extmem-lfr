    ...
    void LFR::_compute_community_assignments() {
        ...
        if (offline_alloc) {
            ...
            for(node_t nid=online_alloc; nid < _number_of_nodes; ++_node_sorter, ++nid) {
                assert(!_node_sorter.empty());
                const auto & ndg = *_node_sorter;
                ndgs.push_back(ndg);
                membership_offsets.push_back(membership_no);
                membership_no += ndg.memberships;
            }
            assert(_node_sorter.empty());
...
void LFR::_verify_assignment() {
        ...
        bool invalid = false;
//#define massert(X) if (!(X)) {std::cerr << " Assertion " #X " failed" << std::endl; invalid = true;}
#define massert(X) assert(X)
        ...
        if (invalid) {
            std::cerr << "Previous assignment is invalid; start over" << std::endl;
            _node_sorter.rewind();
