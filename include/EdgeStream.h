...
protected:
    using em_buffer_t = stxxl::sequence<multinode_t>;
    ...
    // WRITING
    multinode_t _current_out_node;
    edge64id_t _number_of_edges;
...
public:
    Edge64Stream(bool multi_edges = false, bool loops = false)
        : _allow_multi_edges(multi_edges)
        , _allow_loops(loops)
        , _current(edge64_t::invalid())
    {clear();}

    Edge64Stream(const Edge64Stream &) = delete;
    ...
    Edge64Stream(Edge64Stream&&) = default;
    Edge64Stream& operator=(Edge64Stream&&) = default;

// Write interface
    void push(const edge64_t& edge) {
...
// Consume interface
    ...
    Edge64Stream& operator++() {
        ...
        for(; UNLIKELY(*reader == INVALID_MULTINODE); ++reader, ++_current.first) {
